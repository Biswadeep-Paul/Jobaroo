...
const PostJob = () => {
  ...
  // Filter companies to only include those with status "accepted"
  const acceptedCompanies = companies.filter(company => company.status === "accepted");
  ...
  const selectChangeHandler = (value) => {
    const selectedCompany = acceptedCompanies.find((company) => company.name.toLowerCase() === value);
    if (selectedCompany) {
      ...
      <div className='flex items-center justify-center w-screen my-5'>
        <form onSubmit={submitHandler} className='p-8 max-w-4xl border border-gray-200 shadow-lg rounded-md'>
          ...
          {loading ? (
            ...
            <Button type="submit" className="w-full my-4 bg-[#F83002]">Post New Job</Button>
          )}
          {acceptedCompanies.length === 0 && (
